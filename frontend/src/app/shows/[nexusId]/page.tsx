    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-nexus-muted mt-3">
          ...
          {show.content_rating && (
            <span className="rounded border border-nexus-border px-2 py-0.5 text-xs">
              {show.content_rating}
            </span>
          )}
          ...
          <div className="space-y-4">
            {show.seasons.map((season) => (
              <details
                ...
                className="group rounded-xl border border-nexus-border bg-nexus-card"
              >
                <summary className="flex cursor-pointer items-center justify-between p-4 hover:bg-nexus-border/30 transition rounded-xl">
                ...
                <div className="border-t border-nexus-border">
                  {season.episodes.map((ep) => (
                    <div
                      key={ep.nexus_id}
                      className="flex items-center gap-4 border-b border-nexus-border/50 px-4 py-3 last:border-b-0"
                    >
